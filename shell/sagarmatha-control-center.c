...
#include "sagarmatha-control-center.h"
...
struct _SagarmathaControlCenterPrivate
{
  GtkBuilder *builder;
  GtkWidget  *notebook;
  GtkWidget  *main_vbox;
...
static gboolean
activate_panel (SagarmathaControlCenter *shell,
                const gchar        *id,
		const gchar       **argv,
                ...
                GIcon              *gicon)
{
  SagarmathaControlCenterPrivate *priv = shell->priv;
...
static void
_shell_remove_all_custom_widgets (SagarmathaControlCenterPrivate *priv)
{
  GtkBox *box;
  GtkWidget *widget;
  guint i;
...
static void
shell_show_overview_page (SagarmathaControlCenter *center)
{
  SagarmathaControlCenterPrivate *priv = center->priv;
...
void
sagarmatha_control_center_set_overview_page (SagarmathaControlCenter *center)
{
...
static GList *
get_item_views (SagarmathaControlCenter *shell)
{
  GList *list, *l;
  GList *res;
...
search_entry_changed_cb (GtkEntry           *entry,
                         SagarmathaControlCenter *center)
{
  SagarmathaControlCenterPrivate *priv = center->priv;
  char *str;
...
static void
setup_search (SagarmathaControlCenter *shell)
{
...
static void
setup_lock (SagarmathaControlCenter *shell)
{
  SagarmathaControlCenterPrivate *priv = shell->priv;
...
static void
maybe_add_category_view (SagarmathaControlCenter *shell,
                         const char         *name)
{
  GtkTreeModel *filter;
...
static void
reload_menu (SagarmathaControlCenter *shell)
{
  GError *error;
...
static void
setup_model (SagarmathaControlCenter *shell)
{
  SagarmathaControlCenterPrivate *priv = shell->priv;
  ...
  priv->menu_tree = gmenu_tree_new_for_path (MENUDIR "/sagarmathacc.menu", 0);
...
static void
load_panel_plugins (SagarmathaControlCenter *shell)
{
  GList *modules;
...
static void
sagarmatha_control_center_get_property (GObject    *object,
                                   guint       property_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
...
static void
sagarmatha_control_center_set_property (GObject      *object,
                                   guint         property_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
sagarmatha_control_center_dispose (GObject *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_control_center_parent_class)->dispose (object);
}
...
sagarmatha_control_center_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_control_center_parent_class)->finalize (object);
}

static void
sagarmatha_control_center_class_init (SagarmathaControlCenterClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  CcShellClass *shell_class = CC_SHELL_CLASS (klass);

  g_type_class_add_private (klass, sizeof (SagarmathaControlCenterPrivate));

  object_class->get_property = sagarmatha_control_center_get_property;
  object_class->set_property = sagarmatha_control_center_set_property;
  object_class->dispose = sagarmatha_control_center_dispose;
  object_class->finalize = sagarmatha_control_center_finalize;
...
static gint
get_monitor_height (SagarmathaControlCenter *self)
{
  GdkScreen *screen;
  GdkRectangle rect;
...
static gboolean
update_monitor_number (SagarmathaControlCenter *self)
{
...
is_small (SagarmathaControlCenter *self)
{
...
static void
update_small_screen_settings (SagarmathaControlCenter *self)
{
  CcSmallScreen small;
...
static void
sagarmatha_control_center_init (SagarmathaControlCenter *self)
{
  GError *err = NULL;
  SagarmathaControlCenterPrivate *priv;
  GdkScreen *screen;
...
void
sagarmatha_control_center_present (SagarmathaControlCenter *center)
{
  gtk_window_present (GTK_WINDOW (center->priv->window));
}

void
sagarmatha_control_center_show (SagarmathaControlCenter *center,
			   GtkApplication     *app)
{
