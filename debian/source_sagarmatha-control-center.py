...
def add_info(report):
	# the issue is not in the sagarmatha-control-center code so reassign
	if "Stacktrace" in report and "/usr/lib/sagarmatha-control-center-1" in report["Stacktrace"]:
		for words in report["Stacktrace"].split():
			if words.startswith("/usr/lib/sagarmatha-control-center-1"):
			    if apport.packaging.get_file_package(words) != 'sagarmatha-control-center':
	...
	plugin_packages = set()
	for dirpath, dirnames, filenames in os.walk("/usr/lib/sagarmatha-control-center-1"):
		for filename in filenames:
			path = os.path.join(dirpath, filename)
			package = apport.packaging.get_file_package(path)
			if package == 'sagarmatha-control-center':
				continue

			plugin_packages.add(package)

	report["usr_lib_sagarmatha-control-center"] = package_versions(*sorted(plugin_packages))
