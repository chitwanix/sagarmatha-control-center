...
#include <libsagarmatha-desktop/gnome-wall-clock.h>
...
#define CLOCK_SCHEMA "org.sagarmatha.desktop.interface"
...
cc_date_time_panel_init (CcDateTimePanel *self)
{
  ...
  priv->dtm = date_time_mechanism_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM,
                                                          ...
                                                          "org.sagarmatha.SettingsDaemon.DateTimeMechanism",
                                                          "/",
                                                          priv->cancellable,
                                                          &error);
  if (priv->dtm == NULL) {
  ...
  /* add the lock button */
  priv->permission = polkit_permission_new_sync ("org.sagarmatha.settingsdaemon.datetimemechanism.configure", NULL, NULL, NULL);
  if (priv->permission == NULL)
    {
      g_warning ("Your system does not have the '%s' PolicyKit files installed. Please check your installation",
                 "org.sagarmatha.settingsdaemon.datetimemechanism.configure");
      return;
    }
